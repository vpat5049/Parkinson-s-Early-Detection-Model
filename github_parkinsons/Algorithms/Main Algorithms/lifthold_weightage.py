"""
LiftHold Task Weightage Algorithm for Parkinson's Diagnosis
=============================================================
...
LiftHold is a task where the patient holds their arms extended
...
# ============================================================
# For the LiftHold task, we extract 20 features per wrist:
#
...
print("\n" + "=" * 70)
print("STEP 2: Extracting LiftHold features...")
print("=" * 70)

def extract_lifthold_features(data, fs=100.0):
    """Extract 20 features from one wrist's LiftHold recording."""
    acc = data[:, 1:4]
    gyro = data[:, 4:7]
    n = len(data)
    features = []
...
for i, pid in enumerate(patient_ids):
    ...
    # Left wrist
    lpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_LiftHold_LeftWrist.txt')
    if os.path.exists(lpath):
        try:
            ldata = np.loadtxt(lpath, delimiter=',')
            lf = extract_lifthold_features(ldata, FS)
        except:
            lf = [0.0] * N_FEAT
    else:
        lf = [0.0] * N_FEAT
    ...
    # Right wrist
    rpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_LiftHold_RightWrist.txt')
    if os.path.exists(rpath):
        try:
            rdata = np.loadtxt(rpath, delimiter=',')
            rf = extract_lifthold_features(rdata, FS)
        except:
            rf = [0.0] * N_FEAT
    else:
        rf = [0.0] * N_FEAT
...
# ============================================================
print("\n" + "=" * 70)
print("STEP 3: Training Random Forest (LiftHold only)...")
print("=" * 70)
...
print(f"\n  Accuracy (LiftHold task only): {accuracy * 100:.1f}%")
...
print(f"\n  LIFTHOLD SCORING CARD (out of 100)")
...
import argparse
parser = argparse.ArgumentParser(description="LiftLiftHoldage & Diagnosis")
