"""
CrossArms Task Weightage Algorithm for Parkinson's Diagnosis
=============================================================
...
CrossArms is a task where the patient holds their arms extended
...
# ============================================================
# For the CrossArms task, we extract 20 features per wrist:
#
...
print("\n" + "=" * 70)
print("STEP 2: Extracting CrossArms features...")
print("=" * 70)

def extract_crossarms_features(data, fs=100.0):
    """Extract 20 features from one wrist's CrossArms recording."""
    acc = data[:, 1:4]
    gyro = data[:, 4:7]
    n = len(data)
    features = []
...
for i, pid in enumerate(patient_ids):
    ...
    # Left wrist
    lpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_CrossArms_LeftWrist.txt')
    if os.path.exists(lpath):
        try:
            ldata = np.loadtxt(lpath, delimiter=',')
            lf = extract_crossarms_features(ldata, FS)
        except:
            lf = [0.0] * N_FEAT
    else:
        lf = [0.0] * N_FEAT
    ...
    # Right wrist
    rpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_CrossArms_RightWrist.txt')
    if os.path.exists(rpath):
        try:
            rdata = np.loadtxt(rpath, delimiter=',')
            rf = extract_crossarms_features(rdata, FS)
        except:
            rf = [0.0] * N_FEAT
    else:
        rf = [0.0] * N_FEAT
...
# ============================================================
print("\n" + "=" * 70)
print("STEP 3: Training Random Forest (CrossArms only)...")
print("=" * 70)
...
print(f"\n  Accuracy (CrossArms task only): {accuracy * 100:.1f}%")
...
print(f"\n  CROSSARMS SCORING CARD (out of 100)")
...
import argparse
parser = argparse.ArgumentParser(description="CrossArms Weightage & Diagnosis")
