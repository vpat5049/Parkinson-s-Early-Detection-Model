"""
TouchIndex Task Weightage Algorithm for Parkinson's Diagnosis
=============================================================
...
TouchIndex is a task where the patient holds their arms extended
...
# ============================================================
# For the TouchIndex task, we extract 20 features per wrist:
#
...
print("\n" + "=" * 70)
print("STEP 2: Extracting TouchIndex features...")
print("=" * 70)

def extract_touchindex_features(data, fs=100.0):
    """Extract 20 features from one wrist's TouchIndex recording."""
    acc = data[:, 1:4]
    gyro = data[:, 4:7]
    n = len(data)
    features = []
...
for i, pid in enumerate(patient_ids):
    ...
    # Left wrist
    lpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_TouchIndex_LeftWrist.txt')
    if os.path.exists(lpath):
        try:
            ldata = np.loadtxt(lpath, delimiter=',')
            lf = extract_touchindex_features(ldata, FS)
        except:
            lf = [0.0] * N_FEAT
    else:
        lf = [0.0] * N_FEAT
    ...
    # Right wrist
    rpath = os.path.join(TIMESERIES_DIR, f'{pid:03d}_TouchIndex_RightWrist.txt')
    if os.path.exists(rpath):
        try:
            rdata = np.loadtxt(rpath, delimiter=',')
            rf = extract_touchindex_features(rdata, FS)
        except:
            rf = [0.0] * N_FEAT
    else:
        rf = [0.0] * N_FEAT
...
# ============================================================
print("\n" + "=" * 70)
print("STEP 3: Training Random Forest (TouchIndex only)...")
print("=" * 70)
...
print(f"\n  Accuracy (TouchIndex task only): {accuracy * 100:.1f}%")
...
print(f"\n  TOUCHINDEX SCORING CARD (out of 100)")
...
import argparse
parser = argparse.ArgumentParser(description="TouchIndex Weightage & Diagnosis")
